...
void debounce_init(uint8_t num_rows) {
  debounce_counters = (debounce_counter_t *)malloc(num_rows * MATRIX_COLS *
...
bool debounce(matrix_row_t raw[], matrix_row_t cooked[], uint8_t num_rows,
              bool changed) {
  ...
  if (counters_need_update) {
    ...
    if (elapsed_time > 0) {
      update_debounce_counters_and_transfer_if_expired(raw, cooked, num_rows,
                                                       elapsed_time);
    }
  }
      ...
      if (*debounce_pointer != DEBOUNCE_ELAPSED) {
        if (*debounce_pointer <= elapsed_time) {
          *debounce_pointer = DEBOUNCE_ELAPSED;
          matrix_row_t cooked_next = (cooked[row] & ~(ROW_SHIFTER << col)) |
                                     (raw[row] & (ROW_SHIFTER << col));
